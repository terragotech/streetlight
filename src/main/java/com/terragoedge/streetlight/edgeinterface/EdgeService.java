...
public class EdgeService {
    ...
    public JsonObject processEdgeForms_altaspage(EdgeNote edgeNote, List<EdgeFormData> edgeFormDataList, String errorFormTemplateGuid, SlvData slvData,String atlasPage) {
        //updateInstallationForm(edgeFormDataList, slvData, edgeNote);
        //updateInstallationForm_atlaspage(edgeFormDataList, slvData, edgeNote,atlasPage);
        String edgenoteJson = gson.toJson(edgeNote);
        JsonObject edgeJsonObject = (JsonObject) jsonParser.parse(edgenoteJson);
        JsonArray serverEdgeFormJsonArray = edgeJsonObject.get("formData").getAsJsonArray();
        int size = serverEdgeFormJsonArray.size();
        for (int i = 0; i < size; i++) {
            ...
            List<EdgeFormData> formDataList = getEdgeFormData(formDefJson);
            if(formTemplate.equals(errorFormTemplateGuid))
            {
               /*JsonObject jsonObject = serverEdgeFormJsonArray.get(i).getAsJsonObject();
               JsonArray  jsonObject1 = jsonObject.get("formDef").getAsJsonArray();
               System.out.println(jsonObject1);*/
                //updateInstallationForm_atlaspage(formDataList,atlasPage);
...
*/
    ...
    public void updateInstallationForm_atlaspage(List<EdgeFormData> edgeFormDataList,String atlaspage) {
        try {
            /*if (slvData.getMacAddress() != null) {
                //set municipality values from csv
                String macAddress = slvData.getMacAddress().trim();
            ...
            updateFormValue(edgeFormDataList, 8, atlaspage);
        } catch (NumberFormatException e) {
            e.printStackTrace();
        } catch (Exception e) {
            e.printStackTrace();
