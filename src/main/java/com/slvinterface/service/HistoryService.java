...
import com.slvinterface.model.HistoryResponse;
...
public class HistoryService extends AbstractProcessor {
    ...
    public void loadDeviceId(HistoryModel historyModel) {
        try {
            ...
            HttpResponse respone = slvRestService.callGetMethod(url);

            if (respone.getStatusLine().getStatusCode() == 200) {
                String strResponse = slvRestService.getResponseBody(respone);
                logger.info("LoadDevice Respose :" + strResponse);
                String responseString = strResponse;
    ...
    public String getDeviceHistory(HistoryModel historyModel) {
        try {
            if (historyModel.getDeviceId() > 0) {
                ...
                paramsList.add("valueName=" + PropertiesReader.getProperties().getProperty("streetlight.slv.query"));
                paramsList.add("ser=json");
                String params = StringUtils.join(paramsList, "&");
                url = url + "?" + params;
                logger.info("Get MAC Address url :" + url);
                //ResponseEntity<String> response = restService.getRequest(url, true, null);
                String commenturl = commentUrl + "?" + params;
                HttpResponse response = slvRestService.callGetMethod(url);

                if (response.getStatusLine().getStatusCode() == 200) {
                    String strResponse = slvRestService.getResponseBody(response);
                    logger.info("Get MAC Address Respose :" + strResponse);
                    String responseString = strResponse;
